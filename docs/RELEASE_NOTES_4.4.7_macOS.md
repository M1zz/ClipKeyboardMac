...
- 아이폰과 똑같은 카테고리를 맥에서도 쓸 수 있어요, 기본·즐겨찾기·템플릿 같은 기본 제공 카테고리와 직접 만든 카테고리가 아이폰과 같은 순서로 서고, 숨긴 탭은 맥에서도 숨겨집니다. 그동안 맥은 단축어가 들어 있는 카테고리만 보여줬어요. **쓰던 화면이 갑자기 바뀌지 않도록, 이미 쓰고 계신 분께는 먼저 여쭤봅니다** — 목록에 뜨는 안내에서 '맞추기'를 누르면 그때 바뀌고, '지금은 그대로'를 고르면 예전 화면 그대로예요. 환경설정에서 언제든 켜고 끌 수 있습니다
...
- The same categories you set on iPhone, now on Mac, General, Favorites, and built-ins like Templates line up beside your own categories in the same order as on iPhone, and tabs you hid there stay hidden here. Until now the Mac only showed categories that already had snippets in them. **If you are already using the app we ask first** — a note appears above your list, and nothing changes until you choose "Match". Pick "Keep as is" and your screen stays exactly as it was. You can switch it on or off any time in Preferences
...
• 아이폰에서 설정한 카테고리를 맥에서 그대로 — 기존 사용자는 물어본 뒤에만 적용, 환경설정에서 전환 가능
...
• The categories you set on iPhone, mirrored on Mac — existing users are asked first, switchable in Preferences
...
📌 **기존 사용자는 옵트인이다 — 조용히 갈아타지 않는다.**
`MacCategoryTabPreference` 가 판단한다: 단축어가 하나도 없으면(신규 설치) 처음부터 아이폰
구성, 바꿔도 탭이 똑같으면 조용히 맞춤, **둘 다 아니면 배너로 묻고 승낙 전까지 4.4.6 구성
유지**(전체 탭 + 단축어가 든 카테고리만). 환경설정 > General 토글로 언제든 전환.
저장 키는 App Group `mac.category.followPhoneTabs.v1` — 값이 없으면 "아직 안 정함"이다.

⚠️ **승낙하면 "전체" 탭이 없어진다.** iOS 가 없앤 탭이라 맞춘 것이고, 대신 "기본" 탭이
...
✅ **밀려 있던 공유 파일 드리프트 10건을 정리했다 — drift-check 통과 상태로 배포 가능.**
`sync_shared.sh` 로 iOS 원본을 그대로 받았다. 실질 변경은 두 가지뿐이다:
(1) 대시 문자(`—` → `-`) 차이 — 맥 쪽 주석이 언젠가 일괄 치환된 흔적이라 코드 영향 없음,
(2) `UserDefaults(suiteName: AppGroup.identifier)` → `AppGroup.defaults` (같은 suite, 동작 동일).
그 외는 iOS 가 추가한 상수(AppSymbol·DefaultsKey·AppNotification·AppLog.launch)로 전부 가산이다.
⚠️ 맥에만 있던 `DefaultsKey.hasCompletedOnboarding` 은 공유 파일에 둘 수 없어(다음 동기화 때
   지워진다) `ClipKeyboard.tap/MacDefaultsKey.swift` 확장으로 옮겼다. 맥 전용 키는 앞으로 여기.
   `categoryBadgeVisible` 은 쓰는 곳이 없어 그대로 정리됐다.
